...
##########################
export wd="/home/scratch90/bleocata_20241205/MMRF-COMMPASS/Variant_Calling/NEW_6SAMPLES"  # Diretório base do projeto
...
export gdc_token="/home/scratch90/bleocata_20241205/MMRF-COMMPASS/gdc-user-token.2025-01-30T16_17_58.328Z.txt"  # Token de autenticação do GDC
...
mkdir -p "${LOG_DIR}" # Criar diretório de destino, se não existir
...
#        echo "Arquivos já foram baixados anteriormente. Pulando download..." >> "$Pre_log"
#        return 0
#    fi
...
#   else
#       echo "ERRO: Falha ao baixar as amostras do GDC. Verifique seu token ou conexão." >> "$Pre_log"
#        exit 1
#   fi
#}
...
#    mkdir -p "$target_dir"
    ...
    # Encontra arquivos .bam e .bai e cria links simbólicos
#    find "${wd}/Downloaded_manifest" -type f \( -name "*.bam" -o -name "*.bai" \) | while read -r file; do
#        ln -sf "$file" "$target_dir/"  # -s cria link simbólico, -f força sobrescrita se já existir
...
#            mv "${dir}/${antigo}" "${dir}/${dir}/${novo}"  # 🔹 Garante que o novo nome inclua o diretório
...
# Função para processar arquivos .TXT sem incluir o cabeçalho
...
# Função para adicionar o ID da amostra como a primeira coluna na tabela de anotação
New_table (){
  local sample="$1"

  # Obtém o ID da amostra removendo a extensão específica do arquivo BAM
  local id
  id=$(basename "$sample" .rna_seq.genomic.gdc_realn.bam)

  # Define o arquivo de anotação correspondente (com extensão .txt)
  local input_file="$OUTPUT_DIR/annotation/${id}.hg38_multianno.txt"
  local temp_file="$OUTPUT_DIR/annotation/${id}.tmp"

  echo "" >> "${MUTECT_log}"
  echo ">>>>>> Executando New_table para amostra $id: <<<<<< $(date)" >> "${MUTECT_log}"

  # Verifica se o arquivo de entrada existe antes de processar
  if [[ ! -f "$input_file" ]]; then
    echo "Erro: Arquivo de anotação $input_file não encontrado!" >> "${MUTECT_log}"
    return 1
  fi

  # Usa awk para adicionar o ID da amostra como a primeira coluna
  awk -v sample_id="$id" 'NR==1 {print "Sample_ID\t" $0; next} {print sample_id "\t" $0}' "$input_file" > "$temp_file"

  # Substitui o arquivo original pelo modificado
  mv "$temp_file" "$input_file"

  echo "Processado: $id" >> "${MUTECT_log}"
}
export -f New_table
find "$BAM_LIST" -name "*.bam" | parallel -j 4 New_table {}
...
Bam_to_Cram(){
  local sample="$1"
  local id=$(basename "$sample" .rna_seq.genomic.gdc_realn.bam)
  
  echo "" >> "${MUTECT_log}"
  echo ">>>>>> Transformando arquivo $id em CRAM <<<<<< $(date)" >> "${MUTECT_log}"
  
  samtools view -T ${REF_FASTA} -C -o ${wd}/Downloaded_manifest/${id}.cram "${sample}"
  
   if [[ $? -eq 0 ]]; then
      echo ">>>>>> Conversão Bam to Cram concluída com sucesso para: $id <<<<<< $(date)" >> "${MUTECT_log}"
  else
      echo ">>>>>> Erro ao executar Conversão Bam to Cram: $id <<<<<< $(date)" >> "${MUTECT_log}"
      return 1
  fi
}
export -f Bam_to_Sam
find "$wd/ALLsamples" -name "*.bam" | parallel -j 4 Bam_to_Sam {}



# Exclui todos os arquivos .bam encontrados no diretório e subdiretórios
find "$wd" -type f -name "*.bam" -exec rm -f {} \;

echo "Todos os arquivos .bam foram excluídos do diretório e subdiretórios de trabalho."
